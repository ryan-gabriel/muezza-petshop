...
export default function GroomingForm({ grooming, onSubmit, trigger }: GroomingFormProps) {
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      const url = grooming ? `/api/groomings/${grooming.id}` : "/api/groomings";
        ...
        {!showPreview ? (
          <>
            ...
            <div className="px-6 pt-6 pb-2">
              <DialogHeader>
                <DialogTitle>
                  {grooming ? "Edit Grooming Service" : "Add Grooming Service"}
                </DialogTitle>
                <DialogDescription>
                  {grooming
                    ? "Update grooming service details."
                    : "Fill the form to create a grooming service."}
                </DialogDescription>
              </DialogHeader>
            </div>
